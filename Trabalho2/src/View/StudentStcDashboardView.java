...
public class StudentStcDashboardView extends javax.swing.JPanel {
    ...
    public StudentStcDashboardView() {
        initComponents();
        startComboBox();
    }
    ...
    public void startComboBox(){
        ...
        if(userLogged instanceof TeacherModel userAux){
            for (int i = 0; i < ((TeacherModel) userLogged).getClasses().size(); i++) {
                if(!((TeacherModel) userLogged).getClasses().get(i).getStudentsList().isEmpty()){
                    for (int j = 0; j < ((TeacherModel) userLogged).getClasses().get(i).getStudentsList().size(); j++) {
                        studentsComboBox.addItem(((TeacherModel) userLogged).getClasses().get(i).getStudentsList().get(j).getName());
                    }
                }
    ...
    private void initComponents() {
        ...
        studentsComboBox.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Andrew Alexander", "Ted Mosby", "Stefani Joanne", "Katheryn Elizabeth" }));
